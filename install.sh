...
cd /root && yes | bash vst-install.sh --nginx yes --apache yes --phpfpm no --vsftpd no  --proftpd yes --exim yes --dovecot yes --spamassassin no --clamav no  --named yes --iptables yes --fail2ban yes --mysql no --postgresql no  --remi yes --quota no --hostname example.com --email admin@example.com  --password $pass;
